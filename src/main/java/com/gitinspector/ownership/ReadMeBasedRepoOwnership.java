...
public class ReadMeBasedRepoOwnership implements RepoOwnership {
    ...
    private OwnershipInfo parseOwnershipInfo(String repoFullName, String readMeContent) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (ownershipLine.toLowerCase().contains("#ownership#")) {
                ...
                if (i + 2 < lines.length) {
                    // the owner line should look like: "Owner: username (username@homeaway.com)"
                    String ownerLine = lines[i + 1];
                    // only examine the portion after the colon "username (username@homeaway.com)"
                    ...
                    // "Code Reviewers: username (username@homeaway.com), skhatri (skhatri@homeaway.com)"
                    String reviewersLine = lines[i + 2];
                    // only examine the portion after the colon
                    // "username (username@homeaway.com), skhatri (skhatri@homeaway.com)"
                    ...
                    for (String reviewerStr : reviewersParts) {
                        // each reviewer string should look like "username (username@homeaway.com)" or "any"
     ...
     * @param gitUserStr   a String of the form <username> (<email>)</br> e.g. "username (username@homeaway.com)</br>"
