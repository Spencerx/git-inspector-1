...
public class JiraTagJob extends AbstractScheduledTask<Violation> {
    ...
    private static final Logger log = LoggerFactory.getLogger(JiraTagJob.class);
    ...
    public ReportResult<Violation, StringStatistic> execute() throws Exception {
        ...
        for (GHRepository repo : targetRepositories.getTargetedRepositories()) {
            ...
            checkJiraTagsForRepo(reportResult, statsTracker, repo, repoFullName);
        }
    ...
    private void checkJiraTagsForRepo(ReportResult<Violation, StringStatistic> reportResult, GitStatisticsTracker statsTracker,
        GHRepository repo, String repoFullName) throws IOException {
        try {
            ...
            log.error("Failed to check JIRA tags for repo " + repoFullName, e);
        }
